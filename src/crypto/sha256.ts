 ...
 */
export function sha256Buffer(content: ArrayBufferView): Uint8Array
export function sha256Buffer(content: null | string | ArrayBufferView): Uint8Array | null
export function sha256Buffer(content: null | string | ArrayBufferView): Uint8Array | null {
  if (content == null) {
    return null
  }
  ...
  const k = K!; const w = W!; const enc = ENCODER!
  ...
  if (typeof content === 'string') {
    ...
    throw new Error(`[sha256Buffer] Unsupported content type: ${typeof content}`)
  }
  ...
  buf[paddedLen - 8] = hi >>> 24; buf[paddedLen - 7] = hi >>> 16
  buf[paddedLen - 6] = hi >>> 8; buf[paddedLen - 5] = hi
  buf[paddedLen - 4] = lo >>> 24; buf[paddedLen - 3] = lo >>> 16
  buf[paddedLen - 2] = lo >>> 8; buf[paddedLen - 1] = lo
  ...
  for (let offset = 0; offset < paddedLen; offset += 64) {
    ...
    for (let i = 16; i < 64; i++) {
      const v0 = w[i - 15]; const v1 = w[i - 2]
    ...
    for (let i = 0; i < 64; i += 4) {
      for (let j = 0; j < 4; j++) {
        const idx = i + j
        const eS1 = ((e >>> 6) | (e << 26)) ^ ((e >>> 11) | (e << 21)) ^ ((e >>> 25) | (e << 7))
        const t1 = (h + eS1 + (g ^ (e & (f ^ g))) + k[idx] + w[idx]) | 0
        const aS0 = ((a >>> 2) | (a << 30)) ^ ((a >>> 13) | (a << 19)) ^ ((a >>> 22) | (a << 10))
 ...
 */
export function sha256(content: ArrayBufferView): string
export function sha256(content: null | string | ArrayBufferView): string | null
export function sha256(content: null | string | ArrayBufferView): string | null {
  const buffer = sha256Buffer(content as any)
  if (!buffer) {
    return null
  }
